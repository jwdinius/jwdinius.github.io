...
\item
    ...
    \headerrow{\emph{Manager -- Autonomy \& Robotics}}{March 2020 -- Present}
	{\small
	\begin{itemize}[noitemsep]
        \resitem{Transitioned to functional leadership role shortly after joining: (\textit{$\sim$15 direct reports})}
        \begin{itemize}[noitemsep]
                \resitem{Grew the team from 11 to 16 full-time members in my first 10 months}
                \resitem{Developing learning initiatives to grow in-house expertise in new technology foci for the company: \textit{computer vision, machine learning, sensing \&  perception, and visual navigation}}
                \resitem{Initiator and champion of several internal process improvement efforts: \textit{Legacy CM/CI to Gitlab, Docker containers for application development and deployment, and embracing open-source projects for rapid software development}}
                \resitem{Technical lead (upwards of 7 team members) of autonomy R \& D projects (fixed-wing and multirotor).  Responsibilities include program management, application infrastructure development, visual navigation algorithm development, image processing, 3D scene reconstruction (mapping), and visual target detection and tracking}
        \end{itemize}
	\end{itemize}
	}
    \headerrow{\emph{Senior Staff Engineer -- Autonomy \& Robotics}}{December 2019 -- March 2020}
	{\small
	\begin{itemize}[noitemsep]
...
\item
	...
	\begin{itemize}[noitemsep]
        ...
        \resitem{Created a simple multivehicle collision-free path planner using the Hybrid A* algorithm}
	\end{itemize}
	}
\item
...
\item
        ...
        {\small
	\begin{itemize}[noitemsep]
        \resitem{Trained YOLOv4 single-stage object detector using DarkNet}
            \resitem{Deployed trained model in a multithreaded Linux command line application (C++) using OpenCV's Deep Neural Network API}
...
\item
	...
	{\small
	\begin{itemize}[noitemsep]
            \resitem{Created and integrated a simple SLAM-inspired algorithm to increase precision navigation terminal accuracy 5x.  \textit{The net effect was a} $>$5x \textit{decrease in inventory drops.}}
...
\item
	...
	\begin{itemize}[noitemsep]
		...
		\resitem{Built a simulation and multithreaded infrastructure layer for testing the algorithm in a representative environment}
...
\begin{tabular}{ll}
   OS &: Windows, OS X, Ubuntu\\
    ...
    Software \ &: Eigen, Armadillo, Scikit-image, Scikit-learn, OpenCV, Matlab/Simulink, git, \\
    & \ gdb(pdb), cmake, numpy, scipy, pandas, \LaTeX , Boost, IPOPT, ROS, fastai, PyTorch, docker, \\
    & \ Jira, Confluence, Xilinx Vivado, Petalinux, CircleCI, Coveralls \\
